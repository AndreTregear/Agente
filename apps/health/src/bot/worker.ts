...
async function startProvider(): Promise<void> {
  ...
  provider = new BaileysProvider({
    ...
    onConnectionUpdate: (status, phone) => {
      send({ type: 'connection-update', status: status as 'open' | 'connecting' | 'close', phone });
      if (status === 'open') {
        reconnectAttempts = 0;
      }
    },
    ...
    onDisconnect: (_reason, loggedOut) => {
      ...
      if (reconnectAttempts > MAX_RECONNECT_ATTEMPTS) {
        send({ type: 'error', error: `Reconnect attempts exhausted after ${MAX_RECONNECT_ATTEMPTS} tries` });
        if (heartbeatInterval) {
          clearInterval(heartbeatInterval);
          heartbeatInterval = null;
        }
  ...
  try {
    switch (cmd.type) {
      ...
      case 'send-voice-note':
        ...
        try {
          await provider.sendVoiceNote(cmd.jid, cmd.filePath, cmd.duration);
          if (cmd.requestId) {
            send({ type: 'send-result', requestId: cmd.requestId, success: true });
          }
        } catch (err) {
