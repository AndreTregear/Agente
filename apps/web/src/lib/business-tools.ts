...
async function sendMessage(
  ...
  try {
    ...
    // Gateway returned error — fall through to agent
  } catch {
    // Gateway not available — fall through to agent
  }

  // Fallback: queue via OpenClaw agent (System 1.5)
  return sendMessageViaAgent(jid, displayName, message);
}

function sendMessageViaAgent(jid: string, displayName: string, message: string): string {
  const OPENCLAW_BIN =
  ...
  const proc = spawn(
    ...
    [
      'agent',
      '--agent', 'yaya-platform',
      '--message', `Send this exact WhatsApp message to ${jid}: "${message}"`,
      '--local',
      '--thinking', 'off',
      '--timeout', '30',
      '--json',
  ...
  return JSON.stringify({
    ...
    note: 'Message queued — delivering via background agent.',
  });
}
