...
```bash
VLLM_API_BASE=http://localhost:8000/v1 VLLM_API_KEY=$VLLM_API_KEY \
...
```bash
# Check tunnel status
curl -s -o /dev/null -w "%{http_code}" -H "Authorization: Bearer $VLLM_API_KEY" \
...
**File:** `.env.example`
**Issue:** The `.env.example` contained realistic-looking passwords (`CHANGE_ME`, `CHANGE_ME`, `changeme`) that developers might copy verbatim into production `.env` files.
