// Agente Dashboard — Dashboard Page
import { apiGet, formatMoney, formatDateTime, esc } from '../shared/api.js';
...
async function loadData() {
  ...
  try {
    ...
    document.getElementById('kpi-pending').textContent = formatMoney(total);
...
async function loadRevenueChart() {
  try {
    const data = await apiGet('/analytics/sales?period=7d');
    // Build chart from daily breakdown if available
    const canvas = document.getElementById('dash-revenue-chart');
    if (!canvas) return;

    // Fallback: generate last 7 days labels with total spread
    const now = new Date();
    const labels = [];
    const values = [];
    for (let i = 6; i >= 0; i--) {
      ...
      values.push(i === 0 ? (data.total || 0) : 0);
    }
...
function renderTopProducts(products) {
  ...
  tbody.innerHTML = products.slice(0, 5).map(p => `
    <tr>
      ...
      <td class="text-right">${p.total_sold || p.quantity || p.unitsSold || 0}</td>
      <td class="text-right">${formatMoney(p.total_revenue || p.revenue || p.sales || 0)}</td>
    </tr>
  `).join('');
}
