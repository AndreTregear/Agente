// Agente Dashboard — Products Page
  ...
  if (viewMode === 'grid') {
    container.innerHTML = `<div class="product-grid">${products.map(p => `
      <div class="product-card ${!p.active ? 'product-inactive' : ''}">
        ...
        <div class="product-stock">${p.stock == null || p.stock < 0 ? 'Stock ilimitado' : `Stock: ${p.stock}`}</div>
        <div class="product-actions">
          <button class="btn btn-sm btn-secondary" onclick="window._editProduct('${p.id}')">Editar</button>
          <button class="btn btn-sm ${p.active ? 'btn-danger' : 'btn-success'}" onclick="window._toggleProduct('${p.id}', ${p.active ? 0 : 1})">
    ...
    container.innerHTML = `
      <div class="card"><div class="card-body no-pad"><div class="table-wrap">
        <table>
          ...
          <tbody>
            ${products.map(p => `
              <tr class="${!p.active ? 'product-inactive' : ''}">
                <td><strong>${esc(p.name)}</strong></td>
                <td>${esc(p.category || '-')}</td>
                <td class="text-right">${formatMoney(p.price)}</td>
                <td>${p.stock == null || p.stock < 0 ? 'Ilimitado' : p.stock}</td>
                <td><span class="badge ${p.active ? 'badge-delivered' : 'badge-cancelled'}">${p.active ? 'Activo' : 'Inactivo'}</span></td>
                <td>
                  <div class="btn-group">
                    <button class="btn btn-sm btn-secondary" onclick="window._editProduct('${p.id}')">Editar</button>
                    <button class="btn btn-sm ${p.active ? 'btn-danger' : 'btn-success'}" onclick="window._toggleProduct('${p.id}', ${p.active ? 0 : 1})">
...
function showProductModal(product) {
  ...
  AgenteModal.open(isEdit ? 'Editar Producto' : 'Nuevo Producto', `
...
window._toggleProduct = async (id, active) => {
  try {
    if (active === 0) {
      ...
      await apiPut(`/products/${id}`, { active: true });
