...
const ALLOWED_MEDIA_TYPES = new Set([
  ...
  'audio/x-m4a', 'audio/aac', 'application/ogg',
  // Video
...
mediaRouter.post('/upload', upload.single('file'), async (req: Request, res: Response) => {
  try {
    ...
    // Sanitize extension: only allow alphanumeric (prevents path traversal via crafted filenames)
...
mediaRouter.get('/:id', async (req: Request, res: Response) => {
  try {
    const tenantId = getTenantId(req);
    const asset = await mediaAssetsRepo.getAssetById(param(req, 'id'), tenantId);
    if (!asset) {
...
mediaRouter.delete('/:id', async (req: Request, res: Response) => {
  try {
    const tenantId = getTenantId(req);
    const asset = await mediaAssetsRepo.deleteAsset(param(req, 'id'), tenantId);
    if (!asset) {
      res.status(404).json({ error: 'Asset not found' });
      return;
    }
