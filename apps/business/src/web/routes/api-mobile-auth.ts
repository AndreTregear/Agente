...
router.post('/register', validateBody(mobileRegisterSchema), withTimeout(async (req, res) => {
  ...
  try {
    ...
    logger.info({ phone: phone.slice(0, 4) + '****', tenantId: tenant.id }, 'New mobile user registered');
    ...
    logger.error({ err, phone: phone.slice(0, 4) + '****' }, 'Mobile registration failed');
...
router.post('/login', validateBody(mobileLoginSchema), withTimeout(async (req, res) => {
  ...
  logger.info({ phone: phone.slice(0, 4) + '****', tenantId: tenant.id }, 'Mobile user logged in');
