...
// proxy removed — dashboard SPA handles its own routing
...
import { mobileAgenteRouter } from './routes/api-mobile-agente.js';
import { mobileDashboardRouter } from './routes/api-mobile-dashboard.js';
...
import { contadorRouter } from './routes/api-contador.js';
...
export function createWebServer(port: number = 3000): void {
  ...
  app.use('/api/contador/register', authLimiter);
  ...
  // Static frontend files — resolve from project root, not dist/
  // Exclude index.html from static serving (served dynamically with admin phone)
  ...
  // ── Internal dev routes — admin only ──
  ...
  // ── Contador (accountant) portal — registration is public, dashboard requires session+role ──
  ...
  app.use('/api/qr', requireSession, qrRouter);
  ...
  // ── Agente CEO routes (chat, voice, tasks, events) — requires session ──
  app.use('/api/agente', requireSession, agenteRouter);

  // ── Media Server routes — requires session ──
  app.use('/api/v1/media', requireSession, mediaRouter);
  app.use('/api/v1/stream', requireSession, streamRouter);

  // ── Encryption routes ──
  app.use('/api/v1/encryption', requireSession, encryptionRouter);

  // ── Data Warehouse routes — requires session ──
  app.use('/api/v1/warehouse', requireSession, warehouseRouter);

  // ── Mobile / Device routes (auth middleware inside routers) ──
  // Mount specific sub-routers BEFORE the general mobileRouter
  app.use('/api/v1/mobile/agente', mobileAgenteRouter);
  app.use('/api/v1/mobile/dashboard', mobileDashboardRouter);
  app.use('/api/v1/mobile/events', mobileEventsRouter);
  app.use('/api/v1/mobile', mobileRouter);
  app.use('/api/v1/creator', mobileCreatorRouter);

  // ── Dashboard ──
  // The dashboard SPA handles its own auth (login/register screens built-in)
  app.get('/dashboard/{*splat}', (_req, res) => {
    res.sendFile(path.resolve(publicDir, 'dashboard/index.html'));
  });
  ...
  // Contador portal SPA
  app.get('/contador/{*splat}', (_req, res) => {
    res.sendFile(path.resolve(publicDir, 'contador/index.html'));
  });
  app.get('/contador', (_req, res) => {
    res.sendFile(path.resolve(publicDir, 'contador/index.html'));
  });

  app.get('/admin/{*splat}', requireSession, requireAdmin, (_req, res) => {
    res.sendFile(path.resolve(publicDir, 'admin/index.html'));
  });

  // Root — serve landing page with dynamic WhatsApp number from admin's tenant
  app.get('/', async (_req, res) => {
    try {
      const { readFileSync } = await import('node:fs');
      let html = readFileSync(path.resolve(publicDir, 'index.html'), 'utf-8');

      const { getPool } = await import('../db/pool.js');
      const result = await getPool().query(
        `SELECT t.phone FROM tenants t JOIN "user" u ON u."tenantId"::text = t.id::text WHERE u.role = 'admin' AND t.phone IS NOT NULL LIMIT 1`
      );

      const phone = result.rows[0]?.phone;
      if (phone) {
        const digits = phone.replace(/\D/g, '');
        html = html.replace(/19044005702/g, digits);
      }

      res.type('html').send(html);
    } catch (err) {
      logger.debug({ err }, 'Landing page dynamic phone failed, serving static');
