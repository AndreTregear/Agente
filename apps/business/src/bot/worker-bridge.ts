...
import type { WorkerCommand, WorkerEvent, WACallEvent } from '../shared/types.js';
...
export class WorkerBridge {
  ...
  private onCall: ((tenantId: string, events: WACallEvent[]) => void) | null = null;
  ...
  async sendAudio(jid: string, audioBuffer: Buffer, mimetype: string, ptt: boolean = true): Promise<void> {
    const requestId = crypto.randomUUID();
    return new Promise<void>((resolve, reject) => {
      const timer = setTimeout(() => {
        this.pendingRequests.delete(requestId);
  ...
  async sendButtons(
    ...
    return new Promise<void>((resolve, reject) => {
      const timer = setTimeout(() => {
        this.pendingRequests.delete(requestId);
        reject(new Error(`sendButtons timed out after ${WorkerBridge.SEND_TIMEOUT_MS}ms for ${jid}`));
      }, WorkerBridge.SEND_TIMEOUT_MS);

      this.pendingRequests.set(requestId, { resolve, reject, timer });

      try {
        this.send({ type: 'send-buttons', jid, body, buttons, footer, requestId });
      } catch (err) {
        clearTimeout(timer);
        this.pendingRequests.delete(requestId);
        reject(err);
  ...
  async sendList(
    ...
    return new Promise<void>((resolve, reject) => {
      const timer = setTimeout(() => {
        this.pendingRequests.delete(requestId);
        reject(new Error(`sendList timed out after ${WorkerBridge.SEND_TIMEOUT_MS}ms for ${jid}`));
      }, WorkerBridge.SEND_TIMEOUT_MS);

      this.pendingRequests.set(requestId, { resolve, reject, timer });

      try {
        this.send({ type: 'send-list', jid, body, buttonText, sections, footer, requestId });
      } catch (err) {
        clearTimeout(timer);
        this.pendingRequests.delete(requestId);
        reject(err);
  ...
  rejectCall(callId: string, callFrom: string): void {
    ...
    try {
      this.send({ type: 'reject-call', callId, callFrom });
  ...
  private handleWorkerEvent(event: WorkerEvent): void {
    switch (event.type) {
      case 'qr':
        ...
        appBus.emit('qr', this.tenantId, event.dataUrl);
        break;
      ...
      case 'call':
        logger.debug({ tenantId: this.tenantId, callCount: event.events.length }, 'Worker call event');
        this.onCall?.(this.tenantId, event.events);
        break;
