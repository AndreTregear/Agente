...
async function startProvider(): Promise<void> {
  ...
  provider = new BaileysProvider({
    ...
    onCall: (events) => {
      send({ type: 'call', events: events.map(e => ({ ...e, status: e.status as any })) });
    },
  ...
  try {
    switch (cmd.type) {
      ...
      case 'send-audio':
        ...
        try {
          const audioBuffer = Buffer.from(cmd.audioBase64, 'base64');
          await provider.sendAudio(cmd.jid, audioBuffer, cmd.mimetype, cmd.ptt);
          if (cmd.requestId) {
            send({ type: 'send-result', requestId: cmd.requestId, success: true });
          }
        } catch (err) {
      ...
      case 'send-buttons':
        ...
        try {
          await provider.sendButtons(cmd.jid, cmd.body, cmd.buttons, cmd.footer);
          if (cmd.requestId) {
            send({ type: 'send-result', requestId: cmd.requestId, success: true });
          }
        } catch (err) {
      ...
      case 'send-list':
        ...
        try {
          await provider.sendList(cmd.jid, cmd.body, cmd.buttonText, cmd.sections, cmd.footer);
          if (cmd.requestId) {
            send({ type: 'send-result', requestId: cmd.requestId, success: true });
          }
        } catch (err) {
      ...
      case 'reject-call':
        if (provider) {
          await provider.rejectCall(cmd.callId, cmd.callFrom).catch(() => {});
        }
        break;
