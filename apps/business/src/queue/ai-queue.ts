...
import { processWithOpenClaw, processOwnerWithOpenClaw, isOwnerChat } from '../ai/mastra-bridge.js';
...
async function processAIJob(job: Job<AIJobData, AIJobResult>): Promise<AIJobResult> {
  ...
  const { tenantId, channel, jid, pushName, text, imageMediaPath, isVoiceMessage, fromMe } = job.data;
