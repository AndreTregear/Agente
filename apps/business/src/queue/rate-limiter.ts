...
import { createCache, BoundedMap } from '../shared/cache.js';
...
const MAX_TENANT_CACHES = 500; // Cap to prevent unbounded memory growth

// Per-tenant caches (keyed by tenantId, created on demand, LRU-bounded)
const rateCaches = new BoundedMap<string, () => Promise<{ limit: number; windowSec: number }>>(MAX_TENANT_CACHES);
const concurrencyCaches = new BoundedMap<string, () => Promise<number>>(MAX_TENANT_CACHES);
