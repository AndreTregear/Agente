...
export interface ContadorReferral {
  id: number;
  contadorId: number;
  tenantId: string;
  status: string;
...
export interface ContadorCommission {
  id: number;
  contadorId: number;
  tenantId: string;
  sourceType: string;
...
export type WorkerCommand =
  ...
  | { type: 'send-audio'; jid: string; audioBase64: string; mimetype: string; ptt: boolean; requestId?: string }
  | { type: 'send-buttons'; jid: string; body: string; buttons: Array<{ id: string; text: string }>; footer?: string; requestId?: string }
  | { type: 'send-list'; jid: string; body: string; buttonText: string; sections: Array<{ title: string; rows: Array<{ id: string; title: string; description?: string }> }>; footer?: string; requestId?: string }
  | { type: 'send-presence'; jid: string; presenceType: 'composing' | 'paused' }
  | { type: 'reject-call'; callId: string; callFrom: string }
  | { type: 'health-check' };
