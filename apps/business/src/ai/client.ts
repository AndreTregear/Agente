 ...
 *
 * Ley 29733 note: This function creates a temporary ArrayBuffer copy for the
 * fetch FormData. The copy is wiped after the API call completes.
 * The caller is responsible for wiping the original buffer (see audio-compliance.ts).
 */
export async function transcribeAudio(buffer: Buffer, mimetype: string): Promise<string> {
  ...
  // Create a copy for FormData — will be wiped in finally block
  const copyBuf = Buffer.from(buffer);
  try {
    const formData = new FormData();
    const arrayBuf = copyBuf.buffer.slice(copyBuf.byteOffset, copyBuf.byteOffset + copyBuf.byteLength) as ArrayBuffer;
    ...
    // Ley 29733: wipe the internal copy — voice = biometric data
    copyBuf.fill(0);
  }
}
