...
export async function createOrder(
  ...
  const result = await transaction(async (client) => {
    ...
    const customerRow = await client.query<CustomerRow>(
      'SELECT name, jid FROM customers WHERE id = $1 AND tenant_id = $2', [customerId, tenantId],
    );
...
async function getOrderByIdInternal(tenantId: string, id: number): Promise<OrderWithItems | undefined> {
  ...
  const customerRow = await queryOne<CustomerRow>('SELECT name, jid FROM customers WHERE id = $1 AND tenant_id = $2', [row.customer_id, tenantId]);
...
export async function modifyOrder(
  ...
  return transaction(async (client) => {
    ...
    const customerRow = await client.query<CustomerRow>(
      'SELECT name, jid FROM customers WHERE id = $1 AND tenant_id = $2', [orderRow.customer_id, tenantId],
    );
