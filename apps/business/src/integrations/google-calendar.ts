 ...
 */
function generateOAuthState(tenantId: string): string {
  const ts = Date.now().toString();
  const payload = `${tenantId}.${ts}`;
  const hmac = crypto.createHmac('sha256', BETTER_AUTH_SECRET).update(payload).digest('hex').slice(0, 16);
  return `${payload}.${hmac}`;
}

/**
 * Verify HMAC-signed OAuth state. Returns tenantId if valid, null if tampered.
 * Allows up to 10 minutes of clock drift.
 */
export function verifyOAuthState(state: string): string | null {
  const parts = state.split('.');
  if (parts.length !== 3) return null;
  const [tenantId, ts, receivedHmac] = parts;
  // Check timestamp freshness (10 min window)
  const age = Date.now() - parseInt(ts, 10);
  if (isNaN(age) || age < 0 || age > 10 * 60 * 1000) return null;
  const payload = `${tenantId}.${ts}`;
  const expectedHmac = crypto.createHmac('sha256', BETTER_AUTH_SECRET).update(payload).digest('hex').slice(0, 16);
  if (!crypto.timingSafeEqual(Buffer.from(receivedHmac), Buffer.from(expectedHmac))) return null;
  return tenantId;
}
...
export function getAuthUrl(tenantId: string): string {
  ...
  const url = oauth2Client.generateAuthUrl({
    access_type: 'offline',
    scope: SCOPES,
    state: generateOAuthState(tenantId),
