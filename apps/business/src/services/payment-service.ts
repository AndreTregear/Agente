...
export async function syncYapeNotification(
  ...
  // Dedup
  const existing = await yapeNotifRepo.getByHash(tenantId, notificationHash);
  if (existing) {
...
export async function validatePaymentForOrder(
  ...
  return { validated: false, message: 'Payment not yet confirmed. The Agente app will verify it automatically.' };
}
