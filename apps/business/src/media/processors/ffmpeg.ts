...
import { secureDeleteFile } from '../../voice/audio-compliance.js';
...
/**
 * Securely clean up temp file(s) — Ley 29733 compliant.
 * Uses multi-pass overwrite (random + zeros) before unlinking.
 * Audio = biometric data; temp files must never linger with recoverable content.
 */
export async function cleanupTemp(...paths: string[]): Promise<void> {
  for (const p of paths) {
    try {
      await secureDeleteFile(p, 'media-processor');
    } catch {
