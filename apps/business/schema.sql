...
-- Tasks (agente-mobile: owner task management)
...
CREATE INDEX IF NOT EXISTS idx_tasks_tenant_due ON tasks(tenant_id, due_date) WHERE status NOT IN ('completed', 'cancelled');

-- ── Contador (Accountant) Portal ──

-- Contadores — accountant profiles linked to auth users
CREATE TABLE IF NOT EXISTS contadores (
  id SERIAL PRIMARY KEY,
  user_id TEXT NOT NULL REFERENCES "user"(id) ON DELETE CASCADE,
  referral_code VARCHAR(12) UNIQUE NOT NULL,
  company_name VARCHAR(255),
  tax_id VARCHAR(20),  -- RUC
  phone VARCHAR(20),
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_contadores_user ON contadores(user_id);
CREATE UNIQUE INDEX IF NOT EXISTS idx_contadores_referral ON contadores(referral_code);
CREATE INDEX IF NOT EXISTS idx_contadores_status ON contadores(status);

-- Referral tracking — links tenants to the contador who referred them
CREATE TABLE IF NOT EXISTS contador_referrals (
  id SERIAL PRIMARY KEY,
  contador_id INT NOT NULL REFERENCES contadores(id) ON DELETE CASCADE,
  tenant_id UUID NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
  status VARCHAR(20) NOT NULL DEFAULT 'active',  -- active, churned
  referred_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  UNIQUE(contador_id, tenant_id)
);

CREATE INDEX IF NOT EXISTS idx_contador_referrals_contador ON contador_referrals(contador_id);
CREATE INDEX IF NOT EXISTS idx_contador_referrals_tenant ON contador_referrals(tenant_id);

-- Commission ledger — tracks earned commissions from referred tenant payments
CREATE TABLE IF NOT EXISTS contador_commissions (
  id SERIAL PRIMARY KEY,
  contador_id INT NOT NULL REFERENCES contadores(id) ON DELETE CASCADE,
  tenant_id UUID NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
  source_type VARCHAR(30) NOT NULL DEFAULT 'subscription',  -- subscription, one_time
  source_payment_id INT,
  gross_amount NUMERIC(12,2) NOT NULL,      -- tenant payment amount
  commission_rate NUMERIC(4,3) NOT NULL,     -- rate at time of calculation
  commission_amount NUMERIC(12,2) NOT NULL,  -- gross * rate
...
CREATE INDEX IF NOT EXISTS idx_contador_commissions_contador ON contador_commissions(contador_id);
CREATE INDEX IF NOT EXISTS idx_contador_commissions_tenant ON contador_commissions(tenant_id);
CREATE INDEX IF NOT EXISTS idx_contador_commissions_status ON contador_commissions(contador_id, status);
CREATE INDEX IF NOT EXISTS idx_contador_commissions_period ON contador_commissions(contador_id, period_month);

-- Add referral_code column to tenants to track which code brought them
DO $$ BEGIN
  ALTER TABLE tenants ADD COLUMN referred_by VARCHAR(12);
