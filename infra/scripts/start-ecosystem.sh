#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════
# Yaya Platform — Unified Start Script
...
start_pm2_apps() {
    log "Starting PM2 apps (yaya-business, yaya-health)..."
    cd "$HOME"

    if command -v pm2 >/dev/null 2>&1; then
        if [ -f "$HOME/yaya-ecosystem.config.cjs" ]; then
            pm2 start "$HOME/yaya-ecosystem.config.cjs" --env production 2>/dev/null || \
        ...
        warn "PM2 not found, skipping yaya-business and yaya-health"
    fi
...
show_status() {
    ...
    for svc in "${services[@]}"; do
        ...
        if curl -sf -H "Authorization: Bearer ${VLLM_API_KEY:-${AI_API_KEY}}" "$url" >/dev/null 2>&1; then
            echo -e "  ${GREEN}✅${NC} $name  →  $domain"
        else
            echo -e "  ${RED}❌${NC} $name  →  $domain"
        fi
    done
...
stop_all() {
    ...
    cd "$CORE_DIR" && docker compose down 2>/dev/null || true
...
health_check() {
    ...
    local checks=(
        ...
        "vLLM|curl -sf http://localhost:8000/v1/models -H 'Authorization: Bearer ${VLLM_API_KEY:-${AI_API_KEY}}'"
    )
...
case "${1:-start}" in
    ...
    logs)
        ...
        service="${1:-all}"
        if [ "$service" = "all" ]; then
            cd "$CORE_DIR" && docker compose logs -f --tail=50
        else
            cd "$CORE_DIR" && docker compose logs -f --tail=50 "$service" 2>/dev/null || \
