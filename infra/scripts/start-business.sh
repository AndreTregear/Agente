...
DO $$ BEGIN IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'calcom') THEN CREATE ROLE calcom WITH LOGIN PASSWORD 'CHANGE_ME'; END IF; END $$;
...
if [ "$NO_GPU" = false ]; then
  ...
  for i in $(seq 1 60); do
    if curl -s -o /dev/null -w '%{http_code}' http://localhost:8000/v1/models -H "Authorization: Bearer ${AI_API_KEY}" 2>/dev/null | grep -q 200; then
      log "vLLM: ready (${i}0s)"
      break
    fi
    if [ "$i" -eq 60 ]; then
