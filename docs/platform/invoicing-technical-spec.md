...
This document specifies the electronic invoicing subsystem for Agente Platform, starting with Peru (SUNAT) as the v1 implementation and designed with a country-adapter architecture to support Mexico, Ecuador, Colombia, and Argentina in future releases.

The invoicing MCP server (`invoicing-mcp`) integrates with PSE/OSE providers to create, sign, submit, and track electronic tax documents (Comprobantes de Pago Electrónicos) on behalf of Agente Platform clients.
...
| **PSE** (Proveedor de Servicios Electrónicos) | Third-party builds and submits the CPE on behalf of the business | SMBs — **recommended for Agente Platform** |

**Agente Platform uses the PSE path**: we send structured JSON data to a PSE provider's REST API, which handles XML generation, signing, and SUNAT submission. This eliminates the need to manage digital certificates or XML generation ourselves.

#### 1.4.6 Submission Flow

```
┌─────────────┐     ┌──────────────┐     ┌──────────────┐     ┌────────┐
│ Agente Agent   │────▶│ invoicing-mcp│────▶│ PSE Provider │────▶│ SUNAT  │
...
**Recommended for Agente Platform**: Use Nubefact or similar PSE REST API provider for simplicity. The `invoicing-mcp` server abstracts the provider behind an adapter interface.

### 1.6 Auxiliary SUNAT Services

| Service | Description | Use in Agente |
|---------|-------------|-------------|
...
- **Digital certificates**: Managed by the PSE provider, not stored in Agente Platform
