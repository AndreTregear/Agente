...
Every open-source project that powers the Agente Platform. We stand on the shoulders of giants.
...
**What it does:** Subscription billing, usage metering, invoicing, and dunning for the Agente Platform's own B2B billing (charging business owners for their Agente subscriptions).
...
**Acknowledgment:** Kokoro's lightweight architecture delivers natural-sounding Spanish TTS on modest GPU hardware. Voice responses make Agente feel like a real assistant, not a chatbot.
...
*Agente Platform is built entirely on open-source software. We are grateful to every maintainer and contributor who makes these projects possible. If you use Agente, consider supporting these upstream projects — they are the foundation everything runs on.*
