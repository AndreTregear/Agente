...
// Argon2id is an optional peer dependency — import dynamically
let argon2: any;
try {
  argon2 = await import('argon2');
} catch {
  // argon2 not installed — deriveKEK will throw if called
}
 ...
 * Requires 'argon2' as a peer dependency.
 */
export async function deriveKEK(password: string, salt: Buffer): Promise<Buffer> {
  if (!argon2) throw new Error('argon2 is required for deriveKEK — install it as a dependency');
