...
/** Optional session persistence callbacks. */
export interface SessionPersistence {
  updateConnectionStatus(tenantId: string, status: string, phone?: string): Promise<void>;
  updateQrTimestamp(tenantId: string): Promise<void>;
  setSessionError(tenantId: string, error: string): Promise<void>;
}

let sessionPersistence: SessionPersistence | null = null;

/**
 * Set session persistence callbacks for worker bridge.
 */
...
export class WorkerBridge {
  ...
  /** True if heartbeat received within last 30 seconds. */
  isAlive(): boolean {
    if (!this.worker) return false;
    return this.lastHeartbeat > 0 && Date.now() - this.lastHeartbeat < 30_000;
  }

  /** True if the worker thread exists (may or may not be healthy). */
  isRunning(): boolean {
  ...
  async start(databaseUrl: string): Promise<void> {
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      this.worker.on('exit', (code) => {
        ...
        sessionPersistence?.updateConnectionStatus(this.tenantId, 'disconnected').catch(() => {});
      });
  ...
  async startSession(): Promise<void> {
    ...
    sessionPersistence?.updateConnectionStatus(this.tenantId, 'connecting').catch(() => {});
  }
  ...
  async sendVoiceNote(jid: string, filePath: string, duration?: number): Promise<void> {
    const requestId = crypto.randomUUID();
    return new Promise<void>((resolve, reject) => {
      const timer = setTimeout(() => {
        this.pendingRequests.delete(requestId);
        reject(new Error(`sendVoiceNote timed out after ${WorkerBridge.SEND_TIMEOUT_MS}ms for ${jid}`));
      }, WorkerBridge.SEND_TIMEOUT_MS);

      this.pendingRequests.set(requestId, { resolve, reject, timer });

      try {
        this.send({ type: 'send-voice-note', jid, filePath, duration, requestId });
      } catch (err) {
        clearTimeout(timer);
        this.pendingRequests.delete(requestId);
        reject(err);
  ...
  private handleWorkerEvent(event: WorkerEvent): void {
    switch (event.type) {
      case 'qr':
        ...
        sessionPersistence?.updateQrTimestamp(this.tenantId).catch(() => {});
      ...
      case 'connection-update':
        ...
        sessionPersistence?.updateConnectionStatus(
          this.tenantId,
          event.status === 'open' ? 'connected' : event.status === 'connecting' ? 'connecting' : 'disconnected',
          event.phone,
        ).catch(() => {});
      ...
      case 'error':
        ...
        sessionPersistence?.setSessionError(this.tenantId, event.error).catch(() => {});
